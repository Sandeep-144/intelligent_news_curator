// Fetch news from a local JSON file and display on index.html
async function fetchNews() {
    try {
        const response = await fetch('file.json', line=true); // Fetch the local JSON file
        const newsData = await response.json(); // Parse the JSON data
        const newsContainer = document.getElementById('news');

        if (Array.isArray(newsData) && newsData.length > 0) {
            newsData.forEach(item => {
                const title = item.headline; // Use 'headline' from your JSON
                const description = item.short_description; // Use 'short_description'
                const link = item.link; // Use 'link'
                ...
                articleElement.innerHTML = `
                    ...
                    <button onclick="addToInterestedNews('${title}')">Add to Interested</button>
                `;
    ...
    //     // document.getElementById('news').innerHTML = "<p>Error loading news articles.</p>";
    // }
}

// Call fetchNews without parameters if on the index page
if (window.location.pathname === '/index.html') {
    fetchNews(); // Now fetches from local file.json
}
